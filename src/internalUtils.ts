...
export class InternalUtils {
  ...
  static ghostSet<T>(thisArg: any, property: string, value: T, muting?: boolean): TypeStruct_exe_<T> {
    ...
    let manage = _exe_.internal_utils.get_exe_(thisArg).rootManagement
  ...
  static setProperty_strict<T>(thisArg: any, property: string, value: T, muting?: boolean, transformValue: boolean = true): TypeStruct_exe_<T> {
    ...
    let target = _exe_.internal_utils.getByStr(thisArg, property)
    let transformedValue: any = value
    // Check if property exists using 'in' operator (handles undefined values correctly)
    let propertyExists = false
    const management = _exe_.internal_utils.get_exe_(thisArg);
    if (management) {
      if (management.processingType === processingType.map) {
        propertyExists = (thisArg as Map<any, any>).has(property)
      } else if (management.processingType === processingType.set) {
        propertyExists = (thisArg as Set<any>).has(property)
      } else {
        propertyExists = property in thisArg
      }
    }
    let propertyCreated = !propertyExists

    // muting = muting || _exe_.internal_utils.get_exe_(TypeStruct_exe_).mutating
    // typeValue = (muting) ? _exe_.internal_utils.gestType(value) : processingType.noMutation

    typeValue = _exe_.internal_utils.gestType(value)
    typeTarget = _exe_.internal_utils.gestType(target)

    let managementThisArg = _exe_.internal_utils.get_exe_(thisArg);

    // Apply muting default from management if not explicitly provided
    if (muting === undefined) {
      muting = managementThisArg?.mutating ?? true;
    }

    // When muting is false or transformValue is false, assign raw value without proxy wrapping
    if (!muting || !transformValue) {
      transformedValue = value;
    }
    // 1. Manejo de Primitivos
    else if (typeValue === processingType.primitiveData || typeValue === processingType.unset || typeValue === processingType.function) {
      if (_exe_.be(target) && 'value' in _exe_.internal_utils.get_exe_(target).structObj) {
        let internalObj = _exe_.internal_utils.get_exe_(target).structObj;
        ...
        transformedValue = _exe_.internal_utils.newProxy(box, typeValue, thisArg, property);
        propertyCreated = (target === undefined);
      }
    }
    // 2. Manejo de Estructuras Complejas
    else if (!(_exe_.be(value) && 'value' in _exe_.internal_utils.get_exe_(value).structObj)) {
      transformedValue = _exe_.internal_utils.newProxy(value, typeValue, thisArg, property);
      propertyCreated = (target === undefined);
    }
    else {
      transformedValue = value;
  ...
  static gestType(valueTest: any): processingType {
    let detailed = _exe_.internal_utils.gestTypeDetailed(valueTest)
  ...
  static gestTypeDetailed(valueTest: any): string {
    ...
    if (detailed == "object") {
      if (_exe_.be(valueTest) && 'value' in _exe_.internal_utils.get_exe_(valueTest).structObj) {
        return _exe_.internal_utils.gestTypeDetailed(_exe_.internal_utils.get_exe_(valueTest).structObj.value);
      }
      ...
      switch (ctorName) {
        ...
        // case "Object": 
        default: detailed = ctorName; break;
      }
    }
    return detailed
  }
  ...
  static newProxy<T>(thisArg: T, type: processingType, fatherStruct?: TypeStruct_exe_<any>, fatherProperty?: string): TypeStruct_exe_<T> {
          ...
          get(target: object, property: string | symbol, receiver: any) {
            ...
            if (typeof value === "function" && ((target instanceof Set) || (target instanceof Map))) {
              if (property === 'set' || property === 'add') {
                ...
                value = function (propertyKey: any, propertyValue: any) {
                  if (property === 'add') {
                    let had = (target as Set<any>).has(propertyKey)
                    let oldValue = had ? propertyKey : undefined
                    valueBound(propertyKey)
                    if (!had) {
                      management_exe_.rootManagement.callReact(new datChangeObj({
    ...
    if (fatherStruct && fatherProperty) {
      if ([processingType.array, processingType.set, processingType.map].includes(_exe_.internal_utils.gestType(fatherStruct)))
        fatherProperty = '[' + fatherProperty + ']'
      else
        fatherProperty = '|' + fatherProperty
      managementHierarchicalData.path = _exe_.path(fatherStruct) + fatherProperty
      managementHierarchicalData.rootManagement = _exe_.internal_utils.get_exe_(fatherStruct).rootManagement
    ...
    if (thisArg && typeof thisArg === 'object') {
      ...
      // Buffer reactions during initialization to avoid noise
      ...
      if (typeProcessing === processingType.array || typeProcessing === processingType.object) {
        ...
        for (let [key, val] of copyEntries) {
          if (!_exe_.be(val)) {
            let valType = _exe_.internal_utils.gestType(val);
            // Primitives must go through setProperty_strict (wraps in box first)
            if (valType === processingType.primitiveData || valType === processingType.unset || valType === processingType.function) {
              InternalUtils.setProperty_strict(managementHierarchicalData.proxyObj, key, val);
            } else {
              // Objects can go directly to newProxy to preserve object keys
              let proxiedVal = InternalUtils.newProxy(val, valType, managementHierarchicalData.proxyObj, key);
      ...
      // Clear initialization buffer without flushing
      managementHierarchicalData.rootManagement.clearBuffer();
